...
Guide the user through creating a complete, high-quality Antigravity plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.
...
3. Create directory structure using bash:
   ```bash
   mkdir -p plugin-name/.agents/plugins
...
6. Create .gitignore if needed (for .agents/*.local.md, etc.)
...
2. For each command:
   ...
   - Write instructions FOR Antigravity (not TO user)
...
2. Create .mcp.json configuration with:
   ...
   - Command and args (with ${ANTIGRAVITY_PLUGIN_ROOT})
...
3. Create example .agents/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/commands as needed
5. Add to .gitignore: `.agents/*.local.md`
...
5. **Test hook configuration** (if plugin has hooks):
   ...
   - Verify ${ANTIGRAVITY_PLUGIN_ROOT} usage
...
**Goal**: Test that plugin works correctly in Antigravity

**Actions**:
1. **Installation instructions**:
   - Show user how to test locally:
     ```bash
     agy --plugin-dir /path/to/plugin-name
     ```
   - Or copy to `.agents/plugins/` for project testing
...
3. **Testing recommendations**:
   ...
   - For hooks: Use `agy --debug` to see hook execution
...
- **Apply best practices**:
  ...
  - Commands written FOR Antigravity
  - Strong trigger phrases
  - ${ANTIGRAVITY_PLUGIN_ROOT} for portability
