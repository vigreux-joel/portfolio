...
A comprehensive toolkit for developing Antigravity plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality Antigravity plugins:
...
4. **Plugin Settings** - Configuration patterns using .agents/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in Antigravity
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${ANTIGRAVITY_PLUGIN_ROOT}", "block dangerous commands"
...
- ${ANTIGRAVITY_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${ANTIGRAVITY_PLUGIN_ROOT}, user vars)
...
- ${ANTIGRAVITY_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .agents/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using Antigravity's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from Antigravity), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for Antigravity plugins
...
Install from antigravity-marketplace:

```bash
/plugin install plugin-dev@antigravity-marketplace
```

Or for development, use directly:

```bash
agy --plugin-dir /path/to/plugin-dev
```
...
This keeps Antigravity's context focused while providing deep knowledge when needed.
...
- Based on official Antigravity documentation
...
✅ **Portability**
- Use ${ANTIGRAVITY_PLUGIN_ROOT} everywhere
- Relative paths only
...
This plugin is part of the antigravity-marketplace. To contribute improvements:

1. Fork the marketplace repository
2. Make changes to plugin-dev/
3. Test locally with `agy --plugin-dir`
