...
The manifest MUST be in the `.agents/plugins/` directory at the plugin root. Antigravity will not recognize plugins without this file in the correct location.
...
- Plugin identification in Antigravity
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["${ANTIGRAVITY_PLUGIN_ROOT}/servers/github-mcp.js"],
      "env": {
...
When Antigravity loads components:
...
Antigravity validates the manifest on plugin load:
