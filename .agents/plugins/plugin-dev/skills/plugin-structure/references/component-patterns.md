...
1. **Scan enabled plugins**: Read `plugin.json` for each
2. **Discover components**: Look in default and custom paths
3. **Parse definitions**: Read YAML frontmatter and configurations
4. **Register components**: Make available to Antigravity
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during Antigravity initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → Antigravity looks up → Executes
**Agents**: Task arrives → Antigravity evaluates capabilities → Selects agent
**Skills**: Task context matches description → Antigravity loads skill
**Hooks**: Event occurs → Antigravity calls matching hooks
...
**Note**: Antigravity doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, Antigravity doesn't support file references.
...
```bash
#!/bin/bash
source "${ANTIGRAVITY_PLUGIN_ROOT}/lib/test-utils.sh"
run_tests
```
