...
check_script() {
  ...
  if grep -E '^[^#]*/home/|^[^#]*/usr/|^[^#]*/opt/' "$script" | grep -q .; then
    ...
    echo "   Use \$ANTIGRAVITY_PROJECT_DIR or \$ANTIGRAVITY_PLUGIN_ROOT"
    ((warnings++))
  fi

  # Check 8: Uses ANTIGRAVITY_PLUGIN_ROOT
  if ! grep -q "ANTIGRAVITY_PLUGIN_ROOT\|ANTIGRAVITY_PROJECT_DIR" "$script"; then
    echo "💡 Tip: Use \$ANTIGRAVITY_PLUGIN_ROOT for plugin-relative paths"
  fi
