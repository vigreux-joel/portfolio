...
This reference provides common, proven patterns for implementing Antigravity hooks. Use these patterns as starting points for typical hook use cases.
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
FLAG_FILE="$ANTIGRAVITY_PROJECT_DIR/.enable-security-scan"
...
**Note:** Must restart Antigravity after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$ANTIGRAVITY_PROJECT_DIR/.agents/my-plugin.local.json"
...
**Configuration file (.agents/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
