  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/quick-check.sh",
          "timeout": 5
        },
        {
          "type": "prompt",
...
if [ -f ".agents-hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .agents-hooks-config.json)
...
**Example .agents-hooks-config.json:**
```json
{
  "strict_mode": true,
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.agents/audit.log
...
export ANTIGRAVITY_PROJECT_DIR="/tmp/test-project"
export ANTIGRAVITY_PLUGIN_ROOT="$(pwd)"
mkdir -p "$ANTIGRAVITY_PROJECT_DIR"
