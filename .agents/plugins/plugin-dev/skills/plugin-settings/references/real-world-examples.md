...
Detailed analysis of how production plugins use the `.agents/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".agents/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.agents/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".agents/multi-agent-swarm.local.md" > temp.md
mv temp.md ".agents/multi-agent-swarm.local.md"
```
...
Document any changes needed in CONTEXT.md.
```
...
**Purpose:** Prevent session exit and loop Antigravity's output back as input
...
RALPH_STATE_FILE=".agents/ralph-loop.local.md"
...
# Create state file
cat > ".agents/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .agents/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.agents/multi-agent-swarm.local.md` | `.agents/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.agents/my-plugin.local.md"

# GOOD
FILE=".agents/my-plugin.local.md"
```
...
The `.agents/plugin-name.local.md` pattern provides:
