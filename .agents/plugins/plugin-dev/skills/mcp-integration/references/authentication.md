...
Complete guide to authentication methods for MCP servers in Antigravity plugins.
...
Antigravity automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. Antigravity detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by Antigravity
...
No additional auth configuration needed! Antigravity handles everything.
...
Tokens are stored securely by Antigravity:
...
```json
{
  "api": {
    ...
    "headersHelper": "${ANTIGRAVITY_PLUGIN_ROOT}/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "${ANTIGRAVITY_PLUGIN_ROOT}/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "${ANTIGRAVITY_PLUGIN_ROOT}/scripts/generate-jwt.sh"
}
```
