...
Complete reference for all MCP server types supported in Antigravity plugins.
...
{
  "my-server": {
    "command": "${ANTIGRAVITY_PLUGIN_ROOT}/servers/custom-server",
...
1. **Startup**: Antigravity spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire Antigravity session
4. **Shutdown**: Process terminated when Antigravity exits
...
{
  "custom": {
    "command": "${ANTIGRAVITY_PLUGIN_ROOT}/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ${ANTIGRAVITY_PLUGIN_ROOT}**
...
Antigravity handles OAuth flow:
