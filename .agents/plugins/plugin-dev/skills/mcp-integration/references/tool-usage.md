...
Complete guide to using MCP tools effectively in Antigravity plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in Antigravity tools.
...
- Can use any tool Antigravity determines is necessary
...
Antigravity automatically structures tool calls based on schema:

```typescript
// Antigravity generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (Antigravity handles this automatically):
...
2. **Install plugin locally** in `.agents/plugins/`
3. **Verify tools available** with `/mcp`
4. **Test command** that uses tools
5. **Check debug output**: `agy --debug`
