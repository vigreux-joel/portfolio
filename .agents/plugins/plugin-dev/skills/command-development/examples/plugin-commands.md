...
Practical examples of commands designed for Antigravity plugins, demonstrating plugin-specific patterns and features.
...
!`node ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/quality-check.js $1`
...
**Key features:**
- Uses `${ANTIGRAVITY_PLUGIN_ROOT}` for portable path
...
**Security scan:**
!`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/security-scan.sh $1`

**Performance analysis:**
!`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/perf-analyze.sh $1`

**Best practices check:**
!`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/best-practices.sh $1`
...
Template structure: @${ANTIGRAVITY_PLUGIN_ROOT}/templates/api-documentation.md
...
!`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/pre-release-check.sh $1`

**Step 2 - Build artifacts:**
!`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/build-release.sh $1`

**Step 3 - Run test suite:**
!`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/run-tests.sh`

**Step 4 - Package release:**
!`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/package.sh $1`
...
Deployment configuration for $1: @${ANTIGRAVITY_PLUGIN_ROOT}/config/$1-deploy.json
...
- Plugin's linting rules: ${ANTIGRAVITY_PLUGIN_ROOT}/config/lint-rules.json
- Security checklist: ${ANTIGRAVITY_PLUGIN_ROOT}/checklists/security.md
- Performance guidelines: ${ANTIGRAVITY_PLUGIN_ROOT}/docs/performance.md
...
Run plugin analyzer: !`node ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/analyze.js $1`
...
Template: @${ANTIGRAVITY_PLUGIN_ROOT}/templates/review-report.md
...
Check build script exists: !`test -x ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/build.sh && echo "EXISTS" || echo "MISSING"`

Verify configuration available: !`test -f ${ANTIGRAVITY_PLUGIN_ROOT}/config/$1.json && echo "FOUND" || echo "NOT_FOUND"`

If all validations pass:

**Configuration:** @${ANTIGRAVITY_PLUGIN_ROOT}/config/$1.json

**Execute build:** !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/build.sh $1 2>&1`

**Validation results:** !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/validate-build.sh $1 2>&1`
...
Load environment configuration: @${ANTIGRAVITY_PLUGIN_ROOT}/config/$1-checks.json

Determine check level: !`echo "$1" | grep -E "^prod$" && echo "FULL" || echo "BASIC"`

**For production environment:**
- Full test suite: !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/test-full.sh`
- Security scan: !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/security-scan.sh`
- Performance audit: !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/perf-check.sh`
- Compliance check: !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/compliance.sh`

**For non-production environments:**
- Basic tests: !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/test-basic.sh`
- Quick lint: !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/lint.sh`
...
```markdown
Check exists: !`test -f ${ANTIGRAVITY_PLUGIN_ROOT}/path/file && echo "YES" || echo "NO"`
```
...
1. **Test with plugin installed:**
   ```bash
   cd /path/to/plugin
   agy /command-name args
   ```

2. **Verify ${ANTIGRAVITY_PLUGIN_ROOT} expansion:**
   ```bash
   # Add debug output to command
   !`echo "Plugin root: ${ANTIGRAVITY_PLUGIN_ROOT}"`
   ```

3. **Test across different working directories:**
   ```bash
   cd /tmp && agy /command-name
   cd /other/project && agy /command-name
   ```

4. **Validate resource availability:**
   ```bash
   # Check all plugin resources exist
   !`ls -la ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/`
   !`ls -la ${ANTIGRAVITY_PLUGIN_ROOT}/config/`
   ```

### Common Mistakes to Avoid

1. **Using relative paths instead of ${ANTIGRAVITY_PLUGIN_ROOT}:**
   ```markdown
   # Wrong
   !`node ./scripts/analyze.js`

   # Correct
   !`node ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/analyze.js`
   ```
...
2. **Forgetting to allow required tools:**
   ...
   allowed-tools: Bash(*)
   ---
   !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/script.sh`
   ```
...
4. **Hardcoding plugin paths:**
   ...
   @/home/user/.agents/plugins/my-plugin/config.json

   # Correct - works everywhere
   @${ANTIGRAVITY_PLUGIN_ROOT}/config.json
   ```
