...
Comprehensive guidance on creating Antigravity slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Plugin-specific features (${ANTIGRAVITY_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${ANTIGRAVITY_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
Antigravity activates this skill when users:
...
Antigravity loads references and examples as needed based on task.
...
- **Project**: `.agents/commands/` (shared with team)
- **Personal**: `~/.agents/commands/` (your commands)
...
- ✓ Plugin command patterns (${ANTIGRAVITY_PLUGIN_ROOT}, discovery, organization)
...
6. Test examples work with current Antigravity
