...
if [ ! -f ".agents/command-initialized" ]; then
...
Checking for user config: .agents/plugin-name.local.md

if [ -f ".agents/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .agents/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .agents/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .agents/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .agents/plugin-name.local.md to customize"
fi
...
Or set in .agents/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".agents/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .agents/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .agents/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .agents/operation-completed.flag
```
