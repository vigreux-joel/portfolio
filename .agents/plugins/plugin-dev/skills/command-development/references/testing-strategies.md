...
head -n 20 .agents/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .agents/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .agents/commands/*.md

# Check file is in correct location
test -f .agents/commands/my-command.md && echo "Found" || echo "Missing"
```
...
# 5. Check debug logs
tail -f ~/.agents/debug-logs/latest
...
cat > .agents/commands/test-bash.md << 'EOF'
---
...
cat > .agents/commands/test-forbidden.md << 'EOF'
---
...
# Command: .agents/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.agents/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .agents/commands/; then
...
```bash
# Monitor Antigravity during command execution
# In terminal 1:
agy --debug

# In terminal 2:
watch -n 1 'ps aux | grep antigravity'
...
```bash
# Check file location
ls -la .agents/commands/my-command.md

# Check permissions
chmod 644 .agents/commands/my-command.md

# Check syntax
head -n 20 .agents/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .agents/commands/my-command.md
grep '\$ARGUMENTS' .agents/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .agents/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .agents/commands/my-command.md

# Verify command syntax
grep '!\`' .agents/commands/my-command.md
