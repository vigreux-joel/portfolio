...
This reference covers features and patterns specific to commands bundled in Antigravity plugins.

## Table of Contents

- [Plugin Command Discovery](#plugin-command-discovery)
- [ANTIGRAVITY_PLUGIN_ROOT Environment Variable](#antigravity_plugin_root-environment-variable)
...
Antigravity automatically discovers commands in plugins using the following locations:
...
## ANTIGRAVITY_PLUGIN_ROOT Environment Variable

### Purpose

`${ANTIGRAVITY_PLUGIN_ROOT}` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/analyze.js`

Read template: @${ANTIGRAVITY_PLUGIN_ROOT}/templates/report.md
```
...
Lint results: !`node ${ANTIGRAVITY_PLUGIN_ROOT}/bin/lint.js $1`
...
Configuration: @${ANTIGRAVITY_PLUGIN_ROOT}/config/deploy-config.json
...
Use this template: @${ANTIGRAVITY_PLUGIN_ROOT}/templates/api-report.md
...
Step 1 - Prepare: !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/prepare.sh $1`
Step 2 - Config: @${ANTIGRAVITY_PLUGIN_ROOT}/config/$1.json
Step 3 - Execute: !`${ANTIGRAVITY_PLUGIN_ROOT}/bin/execute $1`
...
1. **Always use for plugin-internal paths:**
   ```markdown
   # Good
   @${ANTIGRAVITY_PLUGIN_ROOT}/templates/foo.md
...
2. **Validate file existence:**
   ...
   !`test -f ${ANTIGRAVITY_PLUGIN_ROOT}/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @${ANTIGRAVITY_PLUGIN_ROOT}/config.json
...
4. **Combine with arguments:**
   ```markdown
   Run: !`${ANTIGRAVITY_PLUGIN_ROOT}/bin/process.sh $1 $2`
   ```
...
- Verify syntax is exact: `${ANTIGRAVITY_PLUGIN_ROOT}`
...
Load configuration: @${ANTIGRAVITY_PLUGIN_ROOT}/deploy-config.json
...
Template: @${ANTIGRAVITY_PLUGIN_ROOT}/templates/component-docs.md
...
Validate: !`bash ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/validate.sh`
...
Environment config: @${ANTIGRAVITY_PLUGIN_ROOT}/config/$1.json
...
Cache directory: ${ANTIGRAVITY_PLUGIN_ROOT}/cache/

Analyze @$1 and save results to cache:
!`mkdir -p ${ANTIGRAVITY_PLUGIN_ROOT}/cache && date > ${ANTIGRAVITY_PLUGIN_ROOT}/cache/last-run.txt`
...
- Antigravity will automatically use Task tool to launch agent
...
- Mention skill by name to hint Antigravity should invoke it
...
- Config exists: !`test -f ${ANTIGRAVITY_PLUGIN_ROOT}/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ${ANTIGRAVITY_PLUGIN_ROOT}/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ${ANTIGRAVITY_PLUGIN_ROOT}/bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node ${ANTIGRAVITY_PLUGIN_ROOT}/scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use ${ANTIGRAVITY_PLUGIN_ROOT} for all plugin-internal paths**
